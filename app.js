...
for (let i = 0; i < starsNumber; i++) {
  ...
  stars[i] = {
    starsRadius: (Math.random() * size * canvasEl.width) / 2,
...
function runStars(timeNow) {
  ...
  for (let i = 0; i < starsNumber; i++) {
    ...
    // reposition stars to the other side of screen when they move off either side of the screen
    if (stars[i].horizontalPosition < 0 - stars[i].starsRadius) {
      stars[i].horizontalPosition = canvasEl.width + stars[i].starsRadius;
    } else if (
      stars[i].horizontalPosition >
      canvasEl.width + stars[i].starsRadius
    ) {
      stars[i].horizontalPosition = 0 - stars[i].starsRadius;
    }
    ...
    // reposition stars to top/bottom side of screen when they move off either from the top or bottom
    if (stars[i].verticalPosition < 0 - stars[i].starsRadius) {
      stars[i].verticalPosition = canvasEl.height + stars[i].starsRadius;
    } else if (
      stars[i].verticalPosition >
      canvasEl.height + stars[i].starsRadius
    ) {
      stars[i].verticalPosition = 0 - stars[i].starsRadius;
    }
  }
